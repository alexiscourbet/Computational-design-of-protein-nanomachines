...
def Generate_Pdbs(Nresl,num_to_output,wl_1,wl_2,Rl_1,Rl_2,Nresl_1,num_link_res,orientation,P0_1,P0_2,P1,delta_z,output_file_name,chain_name, chain_order):
...
#############################################################################
    ...
    if t <= Nres_1:
     ...
     supercoil_phase=P0_1[iter]+delta_z[iter]*math.tan(alpha)/(R0_1*deg_to_rad)
...
#      w0_2=wl[iter]/math.sqrt((1+(math.pow(Rl_2[iter],2)-math.pow(Rl_1[iter],2))*math.pow(wl[iter]*math.pi/180,2)/math.pow(1.51,2)))
     ...
     if w0_2==0:
      z1_first_res_of_2nd_seg=d+z_link_last_res
     else:
      z1_first_res_of_2nd_seg=R0_2*w0_2*deg_to_rad/math.tan(alpha)+z_link_last_res
     supercoil_phase_2nd_seg=P0_2[iter]+z1_first_res_of_2nd_seg*math.tan(alpha)/(R0_2*deg_to_rad)
#     a0=(w0_2*(t-1)+supercoil_phase)*deg_to_rad
#     a0=(w0_2*(t-(Nres_1+num_link_res[iter]))+a0_link_last_resi/deg_to_rad)*deg_to_rad
     a0=(w0_2*(t-(Nres_1+num_link_res[iter]))+supercoil_phase_2nd_seg)*deg_to_rad
     print a0/deg_to_rad
...
#     a0=(w0_l*(t-1)+supercoil_phase)*deg_to_rad
     a0=(w0_l*(t-Nres_1)+a0_seg1_last_resi/deg_to_rad+(t-Nres_1)*(P0_2[iter]-P0_1[iter])/num_link_res[iter])*deg_to_rad
     print a0/deg_to_rad
...
##############################################################################
     ...
     a0=(w0_l*(t-t_turn)+a0_turn/deg_to_rad+(t-t_turn)*(P0_2[iter]-P0_1[iter])/num_link_res[iter])*deg_to_rad
     print a0/deg_to_rad
...
def input_params(input):
    ...
    for i in range(num_to_output):
        ...
        P0l_1.append( (float(input[7+offset][0]),float(input[7+offset][1]),int(input[7+offset][2])))
        P0l_2.append( (float(input[8+offset][0]),float(input[8+offset][1]),int(input[8+offset][2])))
        orientation.append( int(input[9+offset][0] ))
        P1l.append( (float(input[10+offset][0]),float(input[10+offset][1]),int(input[10+offset][2])))
        Zl.append( (float(input[11+offset][0]),float(input[11+offset][1]),int(input[11+offset][2])))
    ...
    for i in range(num_to_output):
       ...
       print ' starting P0 for 1st segment:   %s   ending P0:  %s   number of samples:  %s \n'%(P0l_1[i][0],P0l_1[i][1],P0l_1[i][2])
       print ' starting P0 for 2nd segment:   %s   ending P0:  %s   number of samples:  %s \n'%(P0l_2[i][0],P0l_2[i][1],P0l_2[i][2])
...
#        number_of_combinations=number_of_combinations*R0l_1[i][2]*R0l_2[i][2]*P0l[i][2]*P1l[i][2]*P1l_2[i][2]*Zl[i][2]
        ...
        if P0l_1[i][2] > 0. : number_of_combinations=number_of_combinations*P0l_1[i][2]
        if P0l_2[i][2] > 0. : number_of_combinations=number_of_combinations*P0l_2[i][2]
...
# mofify below
    ...
    for i in range(num_to_output):
        ...
        P0_1.append(generate_values(P0l_1[i]))
        P0_2.append(generate_values(P0l_2[i]))
        Z.append(generate_values(Zl[i]))

             
    return(Nres,num_to_output,orientation,R0_1,R0_2,Nres_1,num_link_res,w0_1,w0_2,P0_1,P0_2,P1,Z,output_file_prefix,chain_name,chain_order)
...
Nres,num_to_output,orientation,R0_1,R0_2,Nres_1,num_link_res,w0_1,w0_2,P0_1,P0_2,P1,Z,output_file_prefix,chain_name,chain_order=input_params(input)
items=[]
for i in range(num_to_output):
  ...
  items.append(len(P0_1[i]))
  items.append(len(P0_2[i]))
...
for combo in combos:
    ...
    for i in range(num_to_output):
        if len(w0_1[i])==1 and w0_1[i][0]=="constrained" and R0_1[i][0]=="constrained":
            ...
            w0v_1.append( w0_1[0][id[0]]/math.sqrt(1+(math.pow(R0_1[i][id[2+i*8]],2)-math.pow(R0_1[0][id[2]],2))*math.pow(w0_1[0][id[0]]*math.pi/180,2)/math.pow(1.51,2)))
        else:
            w0v_1.append(w0_1[i][id[0+i*8]])
	    ...
	    w0v_2.append( w0_2[0][id[1]]/math.sqrt(1+(math.pow(R0_2[i][id[3+i*8]],2)-math.pow(R0_1[0][id[3]],2))*math.pow(w0_2[0][id[1]]*math.pi/180,2)/math.pow(1.51,2)))
        else:
            w0v_2.append(w0_2[i][id[1+i*8]])
        if len(R0_1[i])==1 and R0_1[i][0]=="constrained":
            R0v_1.append(R0_1[0][id[2]])
        else:
            R0v_1.append(R0_1[i][id[2+i*8]])
        if len(R0_2[i])==1 and R0_2[i][0]=="constrained":
            R0v_2.append(R0_2[0][id[3]])
        else:
            R0v_2.append(R0_2[i][id[3+i*8]])
        if len(P1[i])==1 and P1[i][0]=="constrained":
            P1v.append(P1[0][id[6]])
        else:
            P1v.append(P1[i][id[6+i*8]])
        P0v_1.append(P0_1[i][id[4+i*8]])
        P0v_2.append(P0_2[i][id[5+i*8]])
        Zv.append(Z[i][id[7+i*8]])           
...
## ##         delta_z.append(-delta_z[0])
    ...
    out_file_name='%s'%(tag)
    for i in range(num_to_output):
      out_file_name=out_file_name+'_%.2f_%.2f_%.2f_%.2f_%.2f_%.2f_%.2f_%.2f'%(w0v_1[i],w0v_2[i],R0v_1[i],R0v_2[i],P0v_1[i],P0v_2[i],P1v[i],Zv[i])
#       out_file_name=out_file_name
    
    out_file_name=out_file_name+'.pdb'
    Generate_Pdbs(Nres,num_to_output,w0v_1,w0v_2,R0v_1,R0v_2,Nres_1,num_link_res,orientation,P0v_1,P0v_2,P1v,Zv,out_file_name,chain_name,chain_order)
